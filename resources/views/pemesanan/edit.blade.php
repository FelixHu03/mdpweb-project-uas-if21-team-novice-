...
<p>Ini halaman edit pemesanan kamar</p>
<div class="row">
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" action="{{ route('pemesanan.update', $pemesanan->id_pemesanan) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="tgl_check_in">Tanggal Check In</label>
                        <input type="date" class="form-control" name="tgl_check_in" id="tgl_check_in" placeholder="Tanggal Check In" value="{{ old('tgl_check_in') ? old('tgl_check_in') : $pemesanan->tgl_check_in }}">
                        @error('tgl_check_in')
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" name="tgl_check_out" id="tgl_check_out" placeholder="Tanggal Check Out" value="{{ old('tgl_check_out') ? old('tgl_check_out') : $pemesanan->tgl_check_out }}">
                        @error('tgl_check_out')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <select name="tipe_kamar" id="tipe_kamar" class="form-control">
                            ...
                            <option value="{{ $kamars->id_kamar }}" data-harga="{{ $kamars->harga }}" {{ $kamars->id_kamar == $pemesanan->id_kamar ? 'selected' : '' }}>{{ $kamars->tipe_kamar }}</option>
                            @endforeach
                        </select>
                        @error('tipe_kamar')
                        <span class="text-danger">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <select name="id_tamu" id="id_tamu" class="form-control">
                            @foreach ($tamu as $tms)
                            <option value="{{ $tms->id_tamu }}" {{ $tms->id_tamu == $pemesanan->id_tamu ? 'selected' : '' }}>{{ $tms->nama }}</option>
                            @endforeach
                        </select>
                        @error('id_tamu')
                        <span class="text-danger">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <select name="layanan_tambahan" id="layanan_tambahan" class="form-control">
                            ...
                            <option value="{{ $layanans->id_layanan }}" data-harga="{{ $layanans->harga_layanan_tambahan }}" {{ $layanans->id_layanan == $pemesanan->layanan_tambahan ? 'selected' : '' }}>{{ $layanans->nama_layanan }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="total_biaya" id="total_biaya" placeholder="Total Biaya" readonly value="{{ old('total_biaya', $pemesanan->total_biaya) }}">
                        @error('total_biaya')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <select name="status_bayar" id="status_bayar" class="form-control">
                            ...
                            <option value="Paid" {{ $pemesanan->status_bayar == 'Paid' ? 'selected' : '' }}>Paid</option>
                            <option value="Unpaid" {{ $pemesanan->status_bayar == 'Unpaid' ? 'selected' : '' }}>Unpaid</option>
                        </select>
                        @error('status_bayar')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
